;(function () {
  ...
  V._isTapQuit = function (t, ch) {
    if (!t || !ch || String(t.goal_rule || ch.goal_rule) !== 'ladder') return false
    const dir = String(t.direction || ch.direction || '')
    const tt = String(t.task_type || ch.task_type || '')
    return dir === 'decrease' && tt === 'counter'
  }

  V._tapQuitArea = function (tt, t, ch) {
    const d = this.data
    ...
    const overs = cap > 0 ? total - cap : 0
    const done = cap > 0 && total > 0 && total <= cap
    const lst = t.today_checkins || []
    let html = '<div class="cp-quit-tally">'
    html += '<div class="cp-quit-tally-head"><div class="cp-quit-tally-num"><b>' + total + '</b><span>/ ' + cap + ' ' + unit + '</span></div>'
    html += '<div class="cp-quit-tally-path">' + t.ladder_start + '→' + t.ladder_goal + ' 梯度 · 第' + (t.day_number || 1) + '天</div></div>'
    html += '<div class="cp-quit-tally-bar"><div class="cp-quit-tally-fill' + (overs > 0 ? ' over' : (done ? ' ok' : '')) + '" style="width:' + (cap > 0 ? Math.min(100, Math.round(total / cap * 100)) : 100) + '%"></div></div>'
    if (overs > 0) html += '<div class="cp-quit-tally-tip warn"><i class="fas fa-hand-holding-heart"></i> 已超今日上限 ' + overs + ' ' + unit + '，今天辛苦了，明天梯度更低，一起稳住</div>'
    else if (done) html += '<div class="cp-quit-tally-tip ok"><i class="fas fa-check-circle"></i> 今日控制在 ' + cap + ' ' + unit + ' 以内，梯度守住</div>'
    else html += '<div class="cp-quit-tally-tip"><i class="fas fa-route"></i> 还可 ' + remaining + ' ' + unit + '，点到即记，系统自动记账</div>'
    html += '<button class="cp-tap-smoke" ' + dis + ' onclick="cpViews.home.doFastTap(1)"><i class="fas fa-smoking"></i><span>我抽了一根</span><em>+1 记账</em></button>'
    html += '<div class="cp-quit-tally-actions">'
    html += '<button class="cp-tap-chip ghost" ' + ((lst.length && !d.checking) ? '' : 'disabled') + ' onclick="cpViews.home.doUndoLast()"><i class="fas fa-rotate-left"></i> 撤销上一根</button>'
